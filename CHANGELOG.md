...
- Subcommands no longer disappear from the manifest of a multi-command Typer
  app. Group detection used `isinstance(cmd, click.Group)`, but Typer vendors
  its own Click internals: as of typer 0.27 with click 8.5,
  `typer.core.TyperGroup` derives from `typer._click.core.Command` and is not a
  `click.Group` subclass. The check returned False, the group was serialised as
  a leaf, and every subcommand was dropped — silently, since a manifest with one
  entry is still a valid manifest. Detection is now by capability
  (`list_commands`), which survives that split.

### Added

- A release workflow: tests on push and pull request, publish on a `v*` tag.
  This repository previously had no CI, which is how 0.2.1 came to be published
